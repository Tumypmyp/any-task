...
pub async fn create_api_key(configuration: &configuration::Configuration, anytype_version: &str, create_api_key_request: models::CreateApiKeyRequest) -> Result<models::CreateApiKeyResponse, Error<CreateApiKeyError>> {
...
pub async fn create_auth_challenge(configuration: &configuration::Configuration, anytype_version: &str, create_challenge_request: models::CreateChallengeRequest) -> Result<models::CreateChallengeResponse, Error<CreateAuthChallengeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_body_create_challenge_request = create_challenge_request;
    ...
    req_builder = req_builder.header("Anytype-Version", p_header_anytype_version.to_string());
    req_builder = req_builder.json(&p_body_create_challenge_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CreateChallengeResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CreateChallengeResponse`")))),
        }
    } else {
        let content = resp.text().await?;
