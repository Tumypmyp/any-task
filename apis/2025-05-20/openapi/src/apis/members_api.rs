...
pub async fn get_member(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, member_id: &str) -> Result<models::ApimodelMemberResponse, Error<GetMemberError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_path_space_id = space_id;
    let p_path_member_id = member_id;

    let uri_str = format!("{}/v1/spaces/{space_id}/members/{member_id}", configuration.base_path, space_id=crate::apis::urlencode(p_path_space_id), member_id=crate::apis::urlencode(p_path_member_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ApimodelMemberResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ApimodelMemberResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_members(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, offset: Option<i32>, limit: Option<i32>) -> Result<models::PaginationPaginatedResponseApimodelMember, Error<ListMembersError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_path_space_id = space_id;
    ...
    let uri_str = format!("{}/v1/spaces/{space_id}/members", configuration.base_path, space_id=crate::apis::urlencode(p_path_space_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PaginationPaginatedResponseApimodelMember`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PaginationPaginatedResponseApimodelMember`")))),
        }
    } else {
        let content = resp.text().await?;
