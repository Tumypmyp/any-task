...
pub async fn add_list_objects(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, list_id: &str, apimodel_add_objects_to_list_request: models::ApimodelAddObjectsToListRequest) -> Result<String, Error<AddListObjectsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_path_space_id = space_id;
    let p_path_list_id = list_id;
    let p_body_apimodel_add_objects_to_list_request = apimodel_add_objects_to_list_request;
    ...
    req_builder = req_builder.json(&p_body_apimodel_add_objects_to_list_request);
...
pub async fn get_list_objects(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, list_id: &str, view_id: &str, offset: Option<i32>, limit: Option<i32>) -> Result<models::PaginationPaginatedResponseApimodelObject, Error<GetListObjectsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_path_space_id = space_id;
    let p_path_list_id = list_id;
...
pub async fn get_list_views(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, list_id: &str, offset: Option<i32>, limit: Option<i32>) -> Result<models::PaginationPaginatedResponseApimodelView, Error<GetListViewsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_path_space_id = space_id;
    let p_path_list_id = list_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PaginationPaginatedResponseApimodelView`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PaginationPaginatedResponseApimodelView`")))),
        }
    } else {
        let content = resp.text().await?;
