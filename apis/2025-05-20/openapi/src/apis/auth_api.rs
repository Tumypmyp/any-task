...
pub async fn create_api_key(configuration: &configuration::Configuration, anytype_version: &str, apimodel_create_api_key_request: models::ApimodelCreateApiKeyRequest) -> Result<models::ApimodelCreateApiKeyResponse, Error<CreateApiKeyError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_body_apimodel_create_api_key_request = apimodel_create_api_key_request;
    ...
    req_builder = req_builder.json(&p_body_apimodel_create_api_key_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ApimodelCreateApiKeyResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ApimodelCreateApiKeyResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn create_auth_challenge(configuration: &configuration::Configuration, anytype_version: &str, apimodel_create_challenge_request: models::ApimodelCreateChallengeRequest) -> Result<models::ApimodelCreateChallengeResponse, Error<CreateAuthChallengeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_header_anytype_version = anytype_version;
    let p_body_apimodel_create_challenge_request = apimodel_create_challenge_request;
    ...
    req_builder = req_builder.header("Anytype-Version", p_header_anytype_version.to_string());
    req_builder = req_builder.json(&p_body_apimodel_create_challenge_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ApimodelCreateChallengeResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ApimodelCreateChallengeResponse`")))),
        }
    } else {
        let content = resp.text().await?;
