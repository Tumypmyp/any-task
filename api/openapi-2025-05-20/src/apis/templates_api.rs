...
pub async fn get_template(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, type_id: &str, template_id: &str) -> Result<models::ApimodelPeriodTemplateResponse, Error<GetTemplateError>> {
    ...
    let uri_str = format!("{}/v1/spaces/{space_id}/types/{type_id}/templates/{template_id}", configuration.base_path, space_id=crate::apis::urlencode(p_space_id), type_id=crate::apis::urlencode(p_type_id), template_id=crate::apis::urlencode(p_template_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ApimodelPeriodTemplateResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ApimodelPeriodTemplateResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_templates(configuration: &configuration::Configuration, anytype_version: &str, space_id: &str, type_id: &str, offset: Option<i32>, limit: Option<i32>) -> Result<models::PaginationPeriodPaginatedResponseApimodelObject, Error<ListTemplatesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_anytype_version = anytype_version;
    let p_space_id = space_id;
    let p_type_id = type_id;
    ...
    let uri_str = format!("{}/v1/spaces/{space_id}/types/{type_id}/templates", configuration.base_path, space_id=crate::apis::urlencode(p_space_id), type_id=crate::apis::urlencode(p_type_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
